class Product {
  // Définition de la classe Product, représente un produit unique
  ...
  factory Product.fromJson(Map<String, dynamic> json) {
    return Product(
      ...
      price: (json['price'] as num)
          .toDouble(), // Récupère le prix (num → double pour être sûr)
      ...
      rating: Rating.fromJson(
          json['rating']), // Récupère l’objet rating via son fromJson
    );
  }
...
class Rating {
  ...
  Rating(
      {required this.rate,
      required this.count}); // Constructeur Rating avec paramètres obligatoires

  // Factory constructor pour créer un Rating à partir d’un JSON
  factory Rating.fromJson(Map<String, dynamic> json) {
    return Rating(
      rate: (json['rate'] as num)
          .toDouble(), // Convertit le champ "rate" en double
